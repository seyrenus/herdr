...
pub(super) fn detect(content: &str) -> AgentState {
    // Blocked
    if content.contains("△ Permission required") || has_ucode_question_prompt(content) {
        return AgentState::Blocked;
    }

    // Working
    if has_interrupt_pattern(&content.to_lowercase())
        || has_ucode_interrupt_footer(content)
        || has_ucode_progress_run(content)
    {
...
fn has_ucode_question_prompt(content: &str) -> bool {
...
fn has_ucode_interrupt_footer(content: &str) -> bool {
...
fn has_ucode_progress_run(line: &str) -> bool {
    let mut run = 0usize;
