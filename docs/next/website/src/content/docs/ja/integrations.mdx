---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Cursor Agent CLI、MastraCode、Grok CLI 向けの Herdr インテグレーションをインストールします。
---
...
| セッション識別 | Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Cursor Agent CLI、Grok CLI | インテグレーションは復元用のネイティブセッション参照を報告します。状態は引き続き Herdr のスクリーンマニフェスト検出から得られます。 |
...
このフックは、Grok CLI、または Grok フック設定を使う互換 Hyper プロセスが Herdr ペイン内で動いている間、Grok の `SessionStart` フックを通じてセッション識別を報告します。Grok の状態は Herdr のスクリーンマニフェスト検出から得られます。

Herdr はデフォルトで `~/.grok` を使い、`GROK_HOME` が設定されていればそちらを使います。Grok の設定ディレクトリはあらかじめ存在している必要があります。Grok 互換 fork は同じ `grok` エージェントとして扱われます。Herdr は Hyper 用の別の設定ディレクトリや環境変数を使いません。Grok はそのディレクトリ内のすべての `hooks/*.json` ファイルをマージするため、インストールは Herdr の `SessionStart` エントリを含む自己完結した `hooks/herdr.json` を `hooks/herdr-agent-state.sh` スクリプトの隣に書き込み、他のフックファイルは編集しません。アンインストールは Herdr が所有するその 2 つのファイルだけを削除します。
