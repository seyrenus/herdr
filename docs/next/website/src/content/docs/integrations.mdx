...
The hook reports session identity through Grok's `SessionStart` hook while Grok CLI, or a compatible Hyper process using the Grok hook config, runs inside a Herdr pane. Grok state comes from Herdr's screen manifest detection.

Herdr uses `~/.grok` by default, or `GROK_HOME` when set. The Grok config directory must already exist. Grok-compatible forks are treated as the same `grok` agent; Herdr does not use a separate Hyper config directory or environment variable. Grok merges every `hooks/*.json` file in that directory, so install writes a self-contained `hooks/herdr.json` with the Herdr `SessionStart` entry next to the `hooks/herdr-agent-state.sh` script, and never edits other hook files. Uninstall removes exactly those two Herdr-owned files.
